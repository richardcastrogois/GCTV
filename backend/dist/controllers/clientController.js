...
const updatePaymentStatus = (req, res) => __awaiter(void 0, void 0, void 0, function* () {
    ...
    try {
        ...
        if (typeof paymentBruto !== "number" ||
            isNaN(paymentBruto) ||
            paymentBruto <= 0) {
            res.status(400).json({
                message: "Valor bruto do pagamento inválido, deve ser um número maior que zero.",
            });
            return;
        }
        if (typeof paymentLiquido !== "number" ||
            isNaN(paymentLiquido) ||
            paymentLiquido <= 0) {
            res.status(400).json({
                message: "Valor líquido do pagamento inválido, deve ser um número maior que zero.",
            });
            return;
        }
        const client = yield prisma.client.findUnique({
        ...
        currentPayments = currentPayments.map((payment) => {
